...
void arm_timer_config_el2(void) {
  ...
  if(GetCoreID() == 0) {
    ...
    LOC_PERIPH->CoreTimerIntControl[3].nCNTPNSIRQ_IRQ = 1;
    // Make sure FIQ is zero
    LOC_PERIPH->CoreTimerIntControl[3].nCNTPNSIRQ_FIQ = 0; 
    // It's already set if starting in EL2, but I still
    // set it for safety. With this value, the divider is 1.
    LOC_PERIPH->TimerPrescaler = 0x80000000 ;    
    ...
    LOC_PERIPH->TimerControlStatus.ReloadValue =
      /* Using a TimerPrescaler of 0x80000000, the frequency taken into
       * account for counting and reloading is the base frequency of
       * 38.4MHz. At this frequency, to make one second, one needs
